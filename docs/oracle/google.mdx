...
```bash
gcloud iam service-accounts keys create ./svc-account.json  --iam-account=svc-account@switchboard-oracle-cluster.iam.gserviceaccount.com
```
...
```bash
gcloud secrets versions add oracle-payer-secret --data-file=authority-keypair.json
```
