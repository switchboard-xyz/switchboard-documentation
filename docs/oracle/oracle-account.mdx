...
- `--queuePubkey` is the oracle queue you will be joining. See [Program](../program) for a list of available queues
...
```bash
ts-node oracle-withdraw --amount=1 --authorityKeypair=secrets/authority-keypair.json \
--oracleKey="${ORACLE_KEY}" --rpcUrl=${RPC_URL}
```

then unwrap your SOL
