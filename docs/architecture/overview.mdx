...
The orchestrator behind a switchboard network and controls how oracles and aggregators are added or removed from the network. Oracles are arranged in a round-robin fashion, where once requested, the next N oracles in the queue are assigned to an aggregator. Oracle positions are periodically swapped to mitigate oracles being assigned to the same feeds each cycle.
